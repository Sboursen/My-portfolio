...
let sectionHeights = projectSections.map((section) => {
  sum += Number(getComputedStyle(section).height.slice(0, -2));
  return sum;
});
...
function showProjectDetails(e) {
  ...
  projectDetails.querySelector(".project-languages").innerHTML = "";

  projectsDetailsData[projectId].technologies.forEach((tech) => {
    const li = document.createElement("li");
    li.textContent = tech;
    projectDetails.querySelector(".project-languages").appendChild(li);
  });

  projectDetails.querySelector(".project-description").textContent =
...
projectDetailsCancelButton.addEventListener("click", hideProjectDetails);
...
function validateContactForm(e) {
  ...
  console.log(isMessageValid, isUsernameValid, isEmailValid);
